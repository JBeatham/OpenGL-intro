...
void OpenGLWindow::initGL()
{
	...
	int normalLoc = glGetAttribLocation(shader, "iNorm");
    glGenBuffers(1, &normalBuffer);
    glBindBuffer(GL_ARRAY_BUFFER, normalBuffer);
    glBufferData(GL_ARRAY_BUFFER, 3*geometry.vertexCount()*sizeof(float), geometry.normalData(), GL_STATIC_DRAW);
	...
	//added
	glm::vec4 lightOneColor = glm::vec4(0.8f,0.0f,0.0f,1.0f);
	int lightOneColorLoc = glGetUniformLocation(shader, "lightOneColor");
	glUniform4fv(lightOneColorLoc,1,&lightOneColor[0]);

	//added
	glm::vec4 lightTwoColor = glm::vec4(0.0f,0.8f,0.0f,1.0f);
	int lightTwoColorLoc = glGetUniformLocation(shader, "lightTwoColor");
	glUniform4fv(lightTwoColorLoc,1,&lightTwoColor[0]);
	

	//added
	glm::vec4 ambColor = glm::vec4(0.0f,1.0f,1.0f,1.0f);
	int ambLoc = glGetUniformLocation(shader, "ambColor");
	glUniform4fv(ambLoc,1,&ambColor[0]);
...
void OpenGLWindow::render()
{
	...
	//added
	glm::vec3 lightOne = glm::vec3(sin(l1x+=0.2),1.0f,sin(l1z+=0.2f));
	int lightOneLoc = glGetUniformLocation(shader, "lightOnePos");
	glUniform3fv(lightOneLoc,1,&lightOne[0]);

	//added
	glm::vec3 lightTwo = glm::vec3(sin(l2x+=0.02),0.8f,sin(l2z+=0.2f));
	int lightTwoLoc = glGetUniformLocation(shader, "lightTwoPos");
	glUniform3fv(lightTwoLoc,1,&lightTwo[0]);
