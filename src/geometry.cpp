...
void GeometryData::loadFromOBJFile(string filename)
{
    ...
    for(int faceIndex=0; faceIndex<tempGeom.faces.size(); faceIndex++)
    {
		...
		if(!hasNormals){
			int posOne = 3*face.vertexIndex[0];
			int posTwo = 3*face.vertexIndex[1];
			int posThree = 3*face.vertexIndex[2];

			int vecOneX = tempGeom.vertices[posOne] - tempGeom.vertices[posTwo];
			int vecOneY = tempGeom.vertices[posOne+1] - tempGeom.vertices[posTwo+1];
			int vecOneZ = tempGeom.vertices[posOne+2] - tempGeom.vertices[posTwo+2];

			int vecTwoX = tempGeom.vertices[posOne] - tempGeom.vertices[posThree];
			int vecTwoY = tempGeom.vertices[posOne+1] - tempGeom.vertices[posThree+1];
			int vecTwoZ = tempGeom.vertices[posOne+2] - tempGeom.vertices[posThree+2];

			int i = vecOneY * vecTwoZ - vecOneZ * vecTwoY;
			int j = vecOneX * vecTwoZ - vecOneZ * vecTwoX; 
			int k = vecOneX * vecTwoY - vecOneY * vecTwoX;
